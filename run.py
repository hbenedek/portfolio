...
def question_1(N: int=200, T: int=300, nb_cells: int=11, path: str="data/nasdaq_raw_2010_2020.csv"):
    """
    How does the out-of-sample risk changes for different shrinkage coefficients?
    For N number of randomly selected stocks with a rolling window of T, the linear shrinkage method is performed,
    for different shrinkage coefficients. The out-of-sample risk is calculated for each shrinkage coefficient and the results are pickled.

    Args:
        N (int): Number of stocks in the portfolio.
        T (int): Length of the rolling window.
        nb_cells (int): Number of alphas to be tested in [0, 1]
        path (str): The path to the file where the data should be loaded from. Defaults to "data/nasdaq_raw_2010_2020.csv".

    Returns:
        The resulting dictionary containing for each alpha the list of out-of-sample risks. 
    """
...
def question_2(N: int = 200, T: int=300, path: str="data/nasdaq_raw_2010_2020.csv"):
    """
    Does higher order clustering and bootsrapping improve HALC?
    For N number of randomly selected stocks with a rolling window of T, the BAHC clustering method is performed,
    for different higher order approximations ranging from 1 to 50, with m=50. The out-of-sample risk is calculated in each case he results are pickled.
    ...
    Returns:
        The resulting dictionary containing for each number of clusters the list of out-of-sample risks.
    """
...
def question_3(nb_cells: int=10, N:int=200, T:int=300, path: str="data/nasdaq_raw_2010_2020.csv"): 
    ...
    Returns:
        The resulting dictionary containing for each cut-off value the list of reliabilities.
    """
...
def question_4(T: int, correlation_estimate: callable, output_path: str, path: str="data/nasdaq_raw_2010_2020.csv") -> None:
    """
...
def question_5(max_T:int =1000, max_N: int=500, Nboot: int=50, nb_cells: int=101, path: str="data/nasdaq_raw_2010_2020.csv"):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--T", type=int, default=300, help="Length of the rolling window")
    parser.add_argument("--method", type=str, default="rtm_clipping", help="Method to use for question 4 (rtm, average, empirical, rie, linear, bahc)")
    parser.add_argument("--nb_cells", type=int, default=10, help="Number of cells for question 2 or 3")
    parser.add_argument("--N", type=int, default=200, help="Number of stocks in the portfolio")
    ...
    if args.question == 0:
        ...
        result = question_1(args.N, args.T, args.nb_cells, args.path)
        ...
        if args.method == "rtm":
            f = rtm_clipped
            output_path = "results/q4_rtm.csv"
        elif args.method == "average":
            f = average_linkage_clustering
            output_path = "results/q4_average.csv"
            ...
            f = bootsrap_halc
            output_path = "results/q4_bahc.csv"
        elif args.method == "rie":
            f = rie_wrapper
            output_path = "results/q4_rie.csv"
        else:
